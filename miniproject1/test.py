...
def train_model(model, train_input, train_target, batch_size, learning_rate=1e-1, epochs=100):
...
# load('', map_location=lambda storage, loc: storage) to change from gpu to cpu
# load('') to load gpu mode
best_model.load_state_dict(torch.load('20_percent.pt', map_location=lambda storage, loc: storage))
best_model.eval()
nb_errors = compute_nb_errors(best_model, test_input, test_target, 1)
print ('########### BEST MODEL ##############')
print ('nb_hidden: {:d} -> error: {:.02f}% {:d} / {:d}'.format(50, 100*nb_errors / test_input.size(0), nb_errors, test_input.size(0)))